...
export default function ModalAddNation (props : any) {
  ...
  const registerNation = async () => {
    ...
    if (nationList.includes(nationKo)) {
      ...
      axios 
        .post(`${MainURL}/tournationcity/registernation`, formData, {
          headers: {
  ...
  const deleteInputLastImage = async (imageName:string) => {
    ...
    axios 
      .post(`${MainURL}/tournationcity/deletenationimage`, {
  ...
  const reviseNation = async () => {
    ...
    axios 
      .post(`${MainURL}/tournationcity/revisenation`, formData, {
        headers: {
          "Content-Type": "multipart/form-data",
        },
        params: getParams,
  ...
  const deleteNation = async (itemId:any, images:any) => {
      ...
      images: JSON.parse(images)
		...
		axios 
			.post(`${MainURL}/tournationcity/deletenation`, getParams)
			.then((res) => {
				if (res.data) {
					alert('삭제되었습니다.');
					props.setRefresh(!props.refresh);
