...
export default function ModalAddScheduleAddAirline (props : any) {
  ...
  const fetchAirlineData = async (tourLocationSelected: string) => {
    const res = await axios.get(`${MainURL}/tourproductschedule/getairplanedata/${scheduleData.nation}/${tourLocationSelected}`);
    if (res.data) {
      const copy = res.data;
      const directAirlineCopy = copy.filter((e:any) => e.sort === 'oneway');
      const viaAirlineCopy = copy.filter((e:any) => e.sort === 'round');
  ...
  const registerPost = async () => {
    ...
    if ( departAirport === '' || departFlight === '') {
      ...
      const getParams = {
        ...
        landCompanyCode: scheduleData.landCompanyCode,
        nation : scheduleData.nation,
        tourLocation: scheduleData.tourLocation,
        tourProductName: scheduleData.tourProductName,
        tourPeriod : scheduleData.tourPeriod,
        tourPeriodCode: scheduleData.tourPeriodCode,
        departAirport: departAirport,
        departFlight: departFlight,
        depositCost: scheduleData.depositCost,
        productNotice : scheduleData.productNotice,
        cautionNote: scheduleData.cautionNote,
        contractBenefit : scheduleData.contractBenefit,
  ...
  const registerDetailPost = async (postId:any, scheduleData: any[]) => {
    for (const item of scheduleData) {
      ...
      try {
        const response = await axios.post(`${MainURL}/tourproductschedule/registerscheduledetail`, getParams);
        if (response.data) {
